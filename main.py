...
def tweet(dialog_number):
    ...
    api = twitter.Api(consumer_key=config.get_consumer_key(), consumer_secret=config.get_consumer_secret(),
                      access_token_key=config.get_access_key(), access_token_secret=config.get_access_secret(),
    ...
    try:
        status = api.PostUpdate(dialogs[dialog_number] + "#4")
